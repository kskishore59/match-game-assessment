...
class Home extends Component {
  ...
  onClickBottomImg = id => {
    const {random, timer} = this.state
    if (random.id === id) {
      this.setState(prev => ({
        score: prev.score + 1,
        random: imagesList[Math.ceil(Math.random() * 30)],
      }))
    } else if (timer === 0) {
  ...
  renderItems = () => {
    const {score, timer, random, activeTabId} = this.state
    const thumbNailItems = imagesList.filter(
    ...
    return (
      <div className="bottom-game-container">
        <div>
          <img src={random.imageUrl} alt="match" className="random-image" />
        </div>
        <div>
        ...
        <div>
          <ul className="thumbNail-items">
            {thumbNailItems.map(each => (
              <ThumbnailItem
                key={each.id}
                details={each}
                onClickBottomImg={this.onClickBottomImg}
              />
            ))}
          </ul>
        </div>
        ...
        <div>
          <nav className="nav-container">
            ...
            <li className="nav-text-container">
              <p className="nav-score-text">
                Score: <span className="nav-timer-text">{score}</span>
              </p>
              <div className="nav-timer-container">
                <img
                  ...
                  alt="timer"
                />
                <p className="nav-timer-text">{timer} sec</p>
              </div>
            </li>
          </nav>
        </div>
        <div className="bottom-container">
          {isGameOver ? this.renderGameOver() : this.renderItems()}
        </div>
      </ul>
    )
  }
